...
class MovieSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // fammi l'attach dei registi e delle categorie
        $movies = Movie::all();
        foreach ($movies as $movie) {
            $directors = \App\Models\Director::pluck('id')->toArray();
            $randomDirectors = collect($directors)->random(rand(1, min(3, count($directors))))->toArray();
            $movie->directors()->attach($randomDirectors);
            $categories = \App\Models\Category::pluck('id')->toArray();
            $randomCategories = collect($categories)->random(rand(1, min(3, count($categories))))->toArray();
